...
local function GetPandemicGlowStyle()
    ...
    -- One-time legacy migration: old string keys or old numeric indices Ã¢â€ â€™ new order
...
-- Secret values from duration objects are passed ONLY to Blizzard widget APIs (SetAlpha) Ã¢â‚¬â€ never compared in Lua
local pandemicCurve
...
local function ApplyPandemicGlow(slot)
    ...
    -- Secret boolean/number Ã¢â€ â€™ EvaluateColorValueFromBoolean Ã¢â€ â€™ SetAlpha (all Blizzard APIs, no Lua comparisons)
...
local function InitDB()
    ...
    -- Migrate debuffTextPosition Ã¢â€ â€™ auraTextPosition (timer position now applies to all auras)
    ...
    -- Migrate unified auraTextPosition Ã¢â€ â€™ per-type timer positions
    ...
    -- Migrate old per-type text color Ã¢â€ â€™ unified auraDurationTextColor
    ...
    -- Migrate showHealthNumber toggle Ã¢â€ â€™ hpNumberPos dropdown
    ...
    -- Migrate old text position keys Ã¢â€ â€™ new slot-based system
    ...
    -- Migrate old global text colors Ã¢â€ â€™ per-slot colors
...
--- Full visual refresh for all plates Ã¢â‚¬â€ called when an entire preset is applied.
...
local function SetupAuraCVars()
    ...
    if NamePlateDriverFrame then
        ...
        -- possible Ã¢â‚¬â€ before our NAME_PLATE_UNIT_ADDED fires.  This prevents
...
-- Map class Ã¢â€ â€™ { powerType, maxPips (fallback) }
...
local function IsQuestMob(unit)
    ...
    -- Skip inside instances Ã¢â‚¬â€ quest mobs are open-world only
...
local function GetReactionColor(unit)
    local db = EllesmereUINameplatesDB
    -- 1. Tapped Ã¢â‚¬â€ always highest
    if UnitIsTapDenied(unit) then
        return db.tapped.r, db.tapped.g, db.tapped.b
    end
    -- 2. Quest mob Ã¢â‚¬â€ second highest
    ...
    -- 3Ã¢â‚¬â€œ4. Threat colors that can NEVER be overwritten:
    --   Ã¢â‚¬Â¢ Non-tank: has aggro, near aggro
    --   Ã¢â‚¬Â¢ Tank: losing aggro, no aggro
    ...
    if InRealInstancedContent() then
        ...
        if status then
            ...
            if not isTankRole then
                -- Non-tank: has aggro / near aggro Ã¢â‚¬â€ absolute priority
                ...
                -- Tank: losing aggro / no aggro Ã¢â‚¬â€ absolute priority
    ...
    -- 9. Tank has aggro (if enabled) Ã¢â‚¬â€ below focus/caster/miniboss
...
local function HideBlizzardFrame(nameplate, unit)
    ...
    if unit and UnitCanAttack("player", unit) then
        ...
        -- Move AurasFrame list frames offscreen Ã¢â‚¬â€ we query C_UnitAuras
        ...
        -- Do NOT unregister events on the Blizzard UnitFrame Ã¢â‚¬â€ we need its
...
function NameplateFrame:SetUnit(unit, nameplate)
    ...
    -- Aura timer positions (per-type: debuffs, buffs, CCs Ã¢â‚¬â€ with "none" to hide)
...
function NameplateFrame:UpdateKickTick(kickProtected, isChannel)
    ...
    -- kickProtected is a secret boolean on Midnight Ã¢â‚¬â€ cannot branch on it.
    ...
    if UnitCastingDuration and self.cast.SetTimerDuration then
        ...
        -- Neither positioner nor marker values are updated Ã¢â‚¬â€ both are set once
        ...
        self._kickTicker = C_Timer.NewTicker(0.1, function()
            ...
            -- Both are secret booleans Ã¢â‚¬â€ chain EvaluateColorValueFromBoolean calls
...
CreatePendingWatcher = function(unit, nameplate)
    ...
    watcher:SetScript("OnEvent", function(self, event, u)
        ...
        -- Unit became attackable Ã¢â‚¬â€ promote to enemy plate
        self:UnregisterAllEvents()
        ...
        -- Watch for the reverse transition (enemy Ã¢â€ â€™ friendly, e.g. duel end)
...
CreateEnemyWatcher = function(unit)
    ...
    watcher:SetScript("OnEvent", function(self, event, u)
        ...
        -- Unit became friendly again Ã¢â‚¬â€ tear down enemy plate, restore to pending
        self:UnregisterAllEvents()
        enemyWatchers[u] = nil
...
-- Single shared UNIT_FACTION handler Ã¢â‚¬â€ avoids N watchers each registering
...
manager:SetScript("OnEvent", function(self, event, unit)
    ...
    if event == "NAME_PLATE_UNIT_ADDED" then
        ...
        if not UnitCanAttack("player", unit) then
            ...
            -- Nameplate frames are recycled Ã¢â‚¬â€ a UF previously used for an enemy
